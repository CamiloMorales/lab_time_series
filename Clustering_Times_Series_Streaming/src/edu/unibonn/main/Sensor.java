...
public class Sensor 
{
	...
	public Sensor(String id, int dimensions) 
	{
		this.id = id;
		this.dimensions = dimensions;
		this.measurements = new double[this.dimensions];
	}

	public Sensor(String id, int dimensions, LocalDateTime from) 
	{
		this.id = id;
	...
	public void generate_measurements()
	{
		...
		{
			this.measurements[i] = SampleGenerator.getInstance().generateMeasurement(initial_record_time.plusHours(i), type);
		}
	}

	public void addMeasurement(int i, double current_erlang)
	{
	...
	{	
		...
		for (int i = 0; i < to_point.getDimensions(); i++)
		{	
	...
	{	
		int dimensions = to_point.getDimensions();
	...
	public void setDimensionality(int dimensions)
	{
		this.dimensions = dimensions;
	}
}